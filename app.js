...
function renderHud() {
  ...
  const percent = Math.round((visitedCount / window.PLANTS.length) * 100);

  el.playerNameLabel.textContent = state.playerName || "Visitante";
  el.visitedCount.textContent = visitedCount;
...
function selectPlant(id, options = { pan: true }) {
