...
const DEBUG_COORDINATE_PICKER = false; // true solo para relevar coordenadas durante edición

// Estadísticas anónimas de uso.
// Esta URL es el Web App de Google Apps Script que escribe eventos en Google Sheets.
const ANALYTICS_URL = "https://script.google.com/macros/s/AKfycbx4g6aOT2Mhv-XbOn7VomziY3dTWH6a9AkNJ4cYmZoTBNVsfh9ixlD6PsVcRAIlpYAPUw/exec";
const ANALYTICS_PROJECT = "jardin-botanico-fagro-demo";
const ANALYTICS_ENABLED = true;
const ANALYTICS_SESSION_KEY = "fagro_botanic_analytics_session_v1";
const APP_VERSION = "2026-06-15-analytics-v2";
...
function trackEvent(eventType, extra = {}) {
  ...
  // Usamos un "pixel" GET para evitar problemas de CORS/redirecciones con Google Apps Script.
  try {
    const params = new URLSearchParams();
    Object.entries(payload).forEach(([key, value]) => {
      if (value === undefined || value === null) return;
      params.set(key, String(value).slice(0, 900));
    });
    params.set("_", `${Date.now()}_${Math.random().toString(16).slice(2)}`);
    ...
    // Mantener una referencia unos segundos para que el navegador no cancele la carga.
    window.__fagroAnalyticsPixels = window.__fagroAnalyticsPixels || [];
    window.__fagroAnalyticsPixels.push(img);
    setTimeout(() => {
      window.__fagroAnalyticsPixels = (window.__fagroAnalyticsPixels || []).filter(x => x !== img);
    }, 10000);
  } catch (error) {
...
function initMap() {
  ...
  // Herramienta para edición interna: si DEBUG_COORDINATE_PICKER = true,
  // clic en el mapa muestra coordenadas para relevar plantas reales. Oculta por defecto.
  if (DEBUG_COORDINATE_PICKER) {
    map.on("click", (event) => {
      const lat = event.latlng.lat.toFixed(6);
      const lng = event.latlng.lng.toFixed(6);
      L.popup()
        .setLatLng(event.latlng)
        .setContent(`<strong>Coordenadas del punto</strong><br><code>lat: ${lat},<br>lng: ${lng}</code>`)
        .openOn(map);
    });
  }
...
function quizFeedbackText(plant, previous) {
  const correctOption = plant.quiz.options[plant.quiz.answerIndex];
  const baseFeedback = (plant.quiz.feedback || "").replace(/^Exacto[\.:]?\s*/i, "");
  if (previous.correct) {
    return `+10 puntos. ${plant.quiz.feedback || "Respuesta correcta."}`;
  }
  return `Respuesta registrada. La respuesta correcta era: “${correctOption}”. ${baseFeedback || "Revisá la ficha y probá con otra estación."}`;
}
...
function renderQuiz(plant) {
  ...
  if (previous) {
    el.quizFeedback.textContent = quizFeedbackText(plant, previous);
...
function nextMission() {
  ...
  if (!pool.length) {
    el.targetStatus.innerHTML = "<strong>¡Recorrido completo!</strong> Reiniciá el juego o explorá nuevamente las plantas.";
